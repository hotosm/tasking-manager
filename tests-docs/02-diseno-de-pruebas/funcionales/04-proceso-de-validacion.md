...
*   **Partición de Equivalencia (Autoría de Mapeo):**
    *   *Clase Válida:* El validador NO es el autor del último estado MAPPED.
    *   *Clase Inválida:* El validador ES el autor del último estado MAPPED.
*   **Transición de Estados (Pre-bloqueo):**
    *   *Transición válida:* De `MAPPED` a `LOCKED_FOR_VALIDATION`.

**C. Casos de Prueba Derivados**
| ID Caso | Pasos de Ejecución | Datos de Entrada (Clases aplicadas) | Resultado Esperado |
| :--- | :--- | :--- | :--- |
| **CP-4001-01** | 1. Iniciar sesión como `Validator`. <br>2. Seleccionar una tarea en estado `MAPPED` de otro usuario.<br>3. Solicitar validación. | **Autoría:** Tercero | El sistema asigna la tarea al Validator, cambiando su estado a `LOCKED_FOR_VALIDATION` y habilitando el panel de evaluación. |
| **CP-4001-02** | 1. Iniciar sesión como `Validator`. <br>2. Seleccionar una tarea en estado `MAPPED` registrada por el propio Validator.<br>3. Intentar validarla. | **Autoría:** Propia | El sistema impide el acceso de validación sobre la tarea y alerta sobre la restricción de validarse a sí mismo. |
...
*   **Partición de Equivalencia (Decisión de Calidad):**
    *   *Clase Válida 1:* Aprobar (Todo correcto).
    *   *Clase Válida 2:* Rechazar / Invalidar (Errores presentes).
*   **Transición de Estados:**
    *   *Transición 1:* De `LOCKED_FOR_VALIDATION` a `VALIDATED`.
    *   *Transición 2:* De `LOCKED_FOR_VALIDATION` a `INVALIDATED`.
...
| ID Caso | Pasos de Ejecución | Datos de Entrada (Clases aplicadas) | Resultado Esperado |
| :--- | :--- | :--- | :--- |
| **CP-4003-01** | 1. Desde el historial de ejecución propio o del proyecto, buscar la tarea recientemente modificada.<br>2. Accionar el botón de Deshacer (Undo) último estado. | **Decisión:** Ejecutar Undo | El sistema revierte la categorización retornando la tarea al estado anterior, anotándolo en sus registros (timeline). |
...
| :--- | :--- | :--- | :--- |
| RF-4001 | ESC-4001 | CP-4001-01 | Partición de Equivalencia, Trans. Estados |
| RF-4002 | ESC-4001 | CP-4001-02 | Partición de Equivalencia |
| RF-4003 | ESC-4002 | CP-4002-01, CP-4002-02 | Partición Equiv., Transición Estados |
| RF-4004 | ESC-4003 | CP-4003-01 | Transición de Estados |