...
const Parameters = {
  ...
  AutoscalingPolicy: {
    ...
    Description:
      "development: min 1, max 1 instance; demo: min 1 max 3 instances; production: min 2 max 9 instances",
  },
  DBSnapshot: {
    Type: "String",
    Description:
      "Specify an RDS snapshot ID, if you want to create the DB from a snapshot.",
    Default: "",
  },
  DatabaseDump: {
    Type: "String",
  ...
  TaskingManagerClientId: {
    Description: "TM_CLIENT_ID",
    Type: "String",
  },
  TaskingManagerClientSecret: {
    Description: "TM_CLIENT_SECRET",
    Type: "String",
  },
  TaskingManagerRedirectUri: {
    Description: "TM_REDIRECT_URI",
    Type: "String",
  },
  TaskingManagerScope: {
    Description: "TM_SCOPE",
    Type: "String",
  },
  ...
  TaskingManagerURL: {
    Description:
      "URL for setting CNAME in Distribution; Ex: example.hotosm.org",
    Type: "String",
    AllowedPattern: "^([a-zA-Z0-9-]*\\.){2}(\\w){2,20}$",
    ConstraintDescription:
      "Parameter must be in the form of a url with subdomain.",
  },
...
const Conditions = {
  ...
  IsTaskingManagerProduction: cf.equals(
    cf.ref("AutoscalingPolicy"),
    "production"
  ),
...
const Resources = {
  TaskingManagerASG: {
    ...
    Properties: {
      ...
      MaxSize: cf.if(
        ...
        9,
        cf.if("IsTaskingManagerDemo", 3, 1)
      ),
      ...
      TargetTrackingConfiguration: {
        ...
        PredefinedMetricSpecification: {
          ...
          ResourceLabel: cf.join("/", [
            ...
            cf.select(5, cf.split(":", cf.ref("TaskingManagerTargetGroup"))),
          ]),
        },
      },
      Cooldown: 300,
  ...
  TaskingManagerLaunchConfiguration: {
            ...
            "/opt/aws/amazon-cloudwatch-agent/etc/amazon-cloudwatch-agent.json":
              {
                content: cf.join("\n", [
                  ...
                  cf.sub(
                    '"log_stream_name": "${AWS::StackName}-cloudwatch-agent.log",'
                  ),
                  '"timezone": "UTC"',
                  "},",
                  "{",
                  cf.sub(
                    '"file_path": "${TaskingManagerLogDirectory}/tasking-manager.log",'
                  ),
                  cf.sub('"log_group_name": "${AWS::StackName}.log",'),
                  ...
                  "{",
                  cf.sub(
                    '"file_path": "${TaskingManagerLogDirectory}/gunicorn-access.log",'
                  ),
                  cf.sub('"log_group_name": "${AWS::StackName}.log",'),
                  cf.sub(
                    '"log_stream_name": "${AWS::StackName}-gunicorn.log",'
                  ),
                  '"timezone": "UTC"',
                  "}]}},",
          ...
          files: {
            ...
            "/etc/cfn/hooks.d/amazon-cloudwatch-agent-auto-reloader.conf": {
              content: cf.join("\n", [
                ...
                cf.sub(
                  "action=cfn-init -v --stack ${AWS::StackName} --resource EC2Instance --region ${AWS::Region} --configsets UpdateEnvironment"
                ),
                "runas=root",
              ]),
              mode: "000400",
    ...
    Properties: {
      ...
      SecurityGroups: [
        cf.importValue(
          cf.join("-", [
            "hotosm-network-production",
            cf.ref("NetworkEnvironment"),
            "ec2s-security-group",
            cf.region,
          ])
        ),
      ...
      UserData: cf.userData([
        ...
        cf.join("", [
          ...
          cf.getAtt("TaskingManagerRDS", "Endpoint.Address"),
        ]),
        ...
        cf.sub('export TM_CLIENT_ID="${TaskingManagerClientId}"'),
        cf.sub('export TM_CLIENT_SECRET="${TaskingManagerClientSecret}"'),
        cf.sub('export TM_REDIRECT_URI="${TaskingManagerRedirectUri}"'),
        cf.sub('export TM_SCOPE="${TaskingManagerScope}"'),
        ...
        cf.if(
          "DatabaseDumpFileGiven",
          cf.sub(
            'aws s3 cp ${DatabaseDump} dump.sql; sudo -u postgres psql "postgresql://$POSTGRES_USER:$POSTGRES_PASSWORD@$POSTGRES_ENDPOINT/$POSTGRES_DB" < dump.sql'
          ),
          ""
        ),
                ...
                Resource: [
                  cf.join("", [
                    ...
                    cf.select(
                      ...
                      cf.split(
                        "/",
                        cf.select(1, cf.split("s3://", cf.ref("DatabaseDump")))
                      )
                    ),
                  ]),
                ],
  ...
  TaskingManagerEC2InstanceProfile: {
    ...
    Properties: {
      Roles: cf.if(
        "DatabaseDumpFileGiven",
        [cf.ref("TaskingManagerDatabaseDumpAccessRole")],
        [cf.ref("TaskingManagerEC2Role")]
      ),
      InstanceProfileName: cf.join("-", [
        cf.stackName,
        "ec2",
        "instance",
        "profile",
  ...
  TaskingManagerTargetGroup: {
    ...
    Properties: {
      ...
      Port: 8000,
      Protocol: "HTTP",
      VpcId: cf.importValue(
        cf.join("-", ["hotosm-network-production", "default-vpc", cf.region])
      ),
  ...
  TaskingManagerRDS: {
    ...
    Properties: {
      ...
      MasterUserPassword: cf.if(
        "UseASnapshot",
        cf.noValue,
        cf.ref("PostgresPassword")
      ),
