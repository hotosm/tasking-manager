...
class GetTaskGeometryApi(Resource):
    ...
    def get(self, project_id, task_id):
        ...
        responses:
            200:
                description: Task geometry found
            404:
                description: Task geometry not found
            500:
                description: Internal Server Error
            """
        try:
            task_geometry = MappingService.get_task_geometry(project_id, task_id)
            return Response(task_geometry, mimetype='application/json', status=200)
        except NotFound:
            return {"Error": "Task Geometry Not Found"}, 404
        except Exception as e:
            error_msg = f'Task GET Task Geometry API - unhandled error: {str(e)}'
            current_app.logger.critical(error_msg)
            return {"Error": error_msg}, 500
