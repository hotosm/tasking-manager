...
class ProjectTasksAPI(Resource):
    ...
    def get(self, project):
        ...
        try:
            ...
            tasks = ProjectService.get_latest_tasks(project_id, preferred_locale, page, page_size,
                                                    sort_by, sort_direction, mapper_name, validator_name,
                                                    status, project_name, as_csv)

            if as_csv:
                export_role = 'csv-single' if single_project else 'csv-multi'
                task_dicts = [task.to_primitive(role=export_role) for task in tasks.tasks]
                out = io.StringIO()
                ...
                return send_file(bytes_out, mimetype='text/csv', as_attachment=True,
                                 attachment_filename=f'project-{str(project_id) if single_project else "multi"}-tasks.csv')
