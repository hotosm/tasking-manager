...
class EnvironmentConfig:
    ...
    ML_ENABLER_URL = os.getenv("TM_ML_ENABLER_URL", None)
    ML_ENABLER_BUILDING_API_ENDPOINT = os.getenv(
        "TM_ML_ENABLER_BUILDING_API_ENDPOINT", None
    )
    ML_ENABLER_LOOKING_GLASS_ENDPOINT = os.getenv(
        "TM_ML_ENABLER_LOOKING_GLASS_ENDPOINT", None
    )
    ML_ENABLER_MAPBOX_TOKEN = os.getenv("TM_ML_ENABLER_MAPBOX_TOKEN", None)
    ML_ENABLER_AGGREGATOR_CLASS = os.getenv(
        "TM_ML_ENABLER_AGGREGATOR_CLASS", "looking_glass"
    )
    ML_ENABLER_ENDPOINTS = {
        "looking_glass": ML_ENABLER_LOOKING_GLASS_ENDPOINT,
        "building_api": ML_ENABLER_BUILDING_API_ENDPOINT,
    }
    ML_TILE_URL = os.getenv("TM_ML_ENABLER_TILE_URL", None)
    ML_OVERPASS_URL = os.getenv("TM_ML_ENABLER_OVERPASS_URL", None)
    ...
    SUPPORTED_LANGUAGES = {
        "codes": os.getenv(
            ...
            "ar, cs, da, de, en, es, fa_IR, fi, fr, hu, gl, id, it, ja, ko, lt, mg, nb, nl_NL, pl, pt, pt_BR, ru, si, sl, ta, uk, vi, zh_TW",  # noqa
        ),
        "languages": os.getenv(
            "TM_SUPPORTED_LANGUAGES",
            "Arabic, Česky, Dansk, Deutsch, English, Español, Persian (Iran), Suomi, Français, Magyar, Galician, Indonesia, Italiano, 日本語, 한국어, Lietuvos, Malagasy, Bokmål, Nederlands, Polish, Português, Português (Brasil), Русский, සිංහල, Slovenščina, தமிழ், Українська, tiếng Việt, 中文",  # noqa
        ),
    }
