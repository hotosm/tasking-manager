...
class InterestService:
    ...
    def compute_contributions_rate(user_id):
        ...
        stmt = TaskHistory.query.with_entities(TaskHistory.project_id)\
            .distinct()\
            .filter(TaskHistory.user_id==user_id).subquery()

        res = db.session.query(
                Interest.name,
                func.count(projects_interests.c.interest_id) / func.sum(func.count(projects_interests.c.interest_id)).over()
            )\
        ...
        rates = [InterestrateDTO({'name': r[0], 'rate': r[1]}) for r in res]
