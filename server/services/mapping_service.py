...
class MappingService:
    ...
    def archive_task(archive_task_dto: ArchiveTaskDTO) -> TaskDTO:
        ...
        task = MappingService.get_task(
            archive_task_dto.task_id, archive_task_dto.project_id
        )
        ...
        user_can_map, error_reason = ProjectService.is_user_permitted_to_map(
            archive_task_dto.project_id, archive_task_dto.user_id
        )
        if not user_can_map:
            if error_reason == MappingNotAllowed.USER_NOT_ACCEPTED_LICENSE:
                raise UserLicenseError("User must accept license to map this task")
    ...
    def unarchive_task(mapped_task: MappedTaskDTO) -> TaskDTO:
        ...
        task = MappingService.get_task_archived_by_user(
            mapped_task.project_id, mapped_task.task_id, mapped_task.user_id
        )
        ...
        if new_state not in [
            ...
            raise MappingServiceError(
                "Can only set status to MAPPED, BADIMAGERY, READY after archived"
            )

        StatsService.update_stats_after_task_state_change(
            mapped_task.project_id, mapped_task.user_id, new_state, mapped_task.task_id
        )
