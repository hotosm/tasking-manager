...
from server.models.postgis.utils import NotFound, InvalidData, InvalidGeoJson, \
                                        parse_box, ST_Extent, ST_Intersects, ST_MakeEnvelope 
...
class ProjectAdminService:
    @staticmethod
    def create_draft_project(draft_project_dto: DraftProjectDTO, ml_enabled: bool = False) -> int:
        """
    ...
    def add_annotations_from_ml(project: Project):
        ...
        for task in task_query.all():                                  
            ...
            task_polygon = ProjectSearchService._make_4326_polygon_from_bbox(bbox_list, 4326)
            response = MLEnablerService.get_prediction_from_bbox('looking_glass', bbox)
            if response.get('status') == 'ok':
                for pred_id in response['prediction_ids']:
                    for prediction in response['predictions'][pred_id]:
                        polygon = ProjectSearchService._make_4326_polygon_from_bbox(prediction['bbox'], 3857)
                        if task_polygon.intersects(polygon):
                            ...
                            TaskAnnotationsService.add_or_update_annotation(annotation, project.id, 'ml') 
