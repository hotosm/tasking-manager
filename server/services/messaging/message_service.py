...
class MessageService:
    ...
    def send_weekly_managers_email():
        ...
        for project in projects:
            ...
            sent = MessageService._send_managers_project_email(
                project, subject, one_week_ago
            )
            if sent:
                messages_sent += 1
    ...
    @staticmethod
    def _send_managers_project_email(
        project: Project,
        subject: str,
        ...
        project_info = project.project_info.first()
        task_histories_query = TaskHistory.query.filter_by(
            project_id=project.id
        ).filter(TaskHistory.action_date.between(min_date, max_date))

        if (
            db.session.query(task_histories_query.exists()).scalar()
            and project.author.email_address
        ):
            message = ""
            num_contributions = 0
            validations = 0
            ...
            SMTPService.send_templated_email(
                project.author.email_address,
                subject,
                "weekly_email_managers_en",
                context,
            )
            return True
        else:
