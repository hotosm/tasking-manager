...
export function FormSubmitButton(props: React.ButtonHTMLAttributes<HTMLButtonElement> & {
  icon?: React.ReactNode,
  loading?: boolean,
  ...
  return (
    <button
      ...
      aria-pressed="false"
      // focusindex="0"
      className={`${disabled ? disabledClassName : className} br1 f5 bn ${disabled || loading ? '' : 'pointer'
        }`}
...
export const CustomButton = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & {
  icon?: React.ReactNode,
  loading?: boolean,
}>(
  (props, ref) => {
...
export function EditButton(props: React.AnchorHTMLAttributes<HTMLAnchorElement> & {
  url: string,
}) {
