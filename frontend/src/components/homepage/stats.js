...
import {
  fetchLocalJSONAPI, fetchExternalJSONAPI
...
export class StatsNumber extends React.Component {
  render() {
    ...
    return(
      <span>
        <FormattedNumber value={ Number(value.substr(0, value.length - 1)) } />{value.substr(-1)}
        </span>
      );
  }
}
...
export class StatsSection extends React.Component {
  ...
  getEditsStats = (event) => {
    this.editsStatsPromise = cancelablePromise(
      fetchExternalJSONAPI(EDITS_API_URL)
    );
    this.editsStatsPromise.promise.then(
      r => {
        this.setState({
          edits: r.edits,
