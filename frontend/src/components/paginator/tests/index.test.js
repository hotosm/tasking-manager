...
it('test if listPageOptions returns the correct options', () => {
  ...
  expect(
    listPageOptions(1, 10)
  ).toEqual([1, 2, 3, '...', 10]);
  expect(
    listPageOptions(2, 10)
  ).toEqual([1, 2, 3, '...', 10]);
  expect(
    listPageOptions(3, 10)
  ).toEqual([1, 2, 3, 4, "...", 10,]);
  expect(
    listPageOptions(5, 10)
  ).toEqual([1, '...', 4, 5, 6, '...', 10]);
  expect(
    listPageOptions(10, 10)
  ).toEqual([1, '...', 8, 9, 10]);
  expect(
    listPageOptions(9, 10)
  ).toEqual([1, '...', 8, 9, 10]);
  expect(
    listPageOptions(8, 10)
  ).toEqual([1, '...', 7, 8, 9, 10]);
});
...
it('test if PageButton returns a button with onClick function and correct children', () => {
  let value = 0
  function updateValue(v) {value = v};
  const element = TestRenderer.create(<PageButton activePage={1} label={3} setPageFn={updateValue} />);
  const testInstance = element.root;
  expect(
    testInstance.findByType('button').findByType('span').children
  ).toEqual(["3"]);
...
it('test if PaginatorLine returns the correct children elements', () => {
  let value = 0
  function updateValue(v) {value = v};
  const element = TestRenderer.create(<PaginatorLine activePage={1} lastPage={5} setPageFn={updateValue} />);
  const testInstance = element.root;
  expect(
    testInstance.findAllByType('button').length
  ).toBe(5);
...
it('test if PaginatorLine returns the correct number of active and inactive buttons', () => {
  ...
  expect(
    testInstance.findAllByProps({className: 'f5 blue-grey'}).length
  ).toBe(1);
  // 3 page links
  expect(
    testInstance.findAllByProps(
      {className: 'bg-white blue-grey f5 br2 base-font button-reset justify-center inline-flex items-center w2 h2 ba ma1 border-box pointer'}
    ).length
  ).toBe(3);
  // active page buttons should be 1
  expect(
    testInstance.findAllByProps(
      {className: 'bg-blue-dark white f5 br2 base-font button-reset justify-center inline-flex items-center w2 h2 ba ma1 border-box pointer'}
    ).length
  ).toBe(1);
});
