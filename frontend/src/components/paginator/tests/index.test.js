...
it('test if PaginatorLine returns the correct number of active and inactive buttons', () => {
  ...
  expect(
    testInstance.findAllByProps({
      className:
        'bg-white blue-grey f6 br1 base-font button-reset justify-center bn border-box shadow-3 pointer paginator-btn',
    }).length,
  ).toBe(3);
  ...
  expect(
    testInstance.findAllByProps({
      className:
        'bg-blue-dark white f6 br1 base-font button-reset justify-center bn border-box shadow-3 pointer paginator-btn',
    }).length,
  ).toBe(1);
});
