...
export const projectCardPlaceholderTemplate = (cardWidthClass="w-25-l") => (n,i) => (
    ...
    <div className="w-50 red dib"> <MediaBlock rows={1} className="show-loading-animation" color='#DDD' style={{width: 60, height: 30}}/> </div>
    <div className={`fr w-33 tc pr4 f7 ttu`}> <RectShape className="show-loading-animation" color='#DDD' style={{width: 60, height: 30}}/>   </div>
...
export const nCardPlaceholders = (N, cardWidthClass="w-25-l") => {
  return [ ...Array(N).keys() ].map(
    projectCardPlaceholderTemplate(cardWidthClass)
 );
}
