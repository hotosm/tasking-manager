...
export function ProjectCard({
  ...
  const bottomButtons = (
    ...
    <Link to={`/manage/projects/${projectId}`} className={`fl di f7 di w-50 tc bg-grey-light hover-dark-gray blue-grey bn ${linkCombo}`}>Project Page</Link>
    <Link to={`/projects/${projectId}/map`} className={`fr di f7 di w-50 tc bg-red white hover-moon-gray  bn ${linkCombo}`}>Select Task</Link>
    </div>
  );

  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className={`relative fl ${cardWidthClass} base-font w-50-m w-100 mb3 ${bottomButtonMargin} ph2 blue-dark mw5 `}>
