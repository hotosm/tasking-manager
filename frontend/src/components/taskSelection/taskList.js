  ...
  return (
    <span>
      <span className={`${['READY', 'LOCKED_FOR_MAPPING'].includes(status) && 'ba b--grey-light'} dib`}
        style={{height: dotSize, width: dotSize, borderWidth: "2px", backgroundColor: TASK_COLOURS[status]}} >
      </span>
...
function TaskItem({data, projectId, selectTask, selected=[]}: Object) {
  return (
    <div className={`cf db ba br1 mt2 ${selected.includes(data.taskId) ? 'b--blue-dark bw1' : 'b--tan bw1'}`}>
      <div className="w-90 pv3 fl cf pointer" onClick={() => selectTask(data.taskId, data.taskStatus)} >
        <div className="w-75 fl dib">
        ...
        <div className="w-25 fl blue-grey dib">
          <TaskStatus status={data.taskStatus} />
        </div>
      </div>
      <div className="w-10 pv3 fl dib blue-light">
        <div className="pl1 dib v-mid">
...
export function TaskList({project, activeFilter, selectTask, selected}: Object) {
  const [tasksError, tasksLoading, tasks] = useFetch(`projects/${project.projectId}/activities/latest/`);
  const user = useSelector(state => state.auth.get('userDetails'));
      ...
      if (tasks && tasks.activity) {
        ...
        if (textSearch) {
          newTasks = newTasks.filter(
            task => task.taskId === Number(textSearch) || (task.actionBy && task.actionBy.includes(textSearch))
          );
        }
        setTasks(newTasks);
      }
  ...
  return <div className="cf">
    <div className="cf">
      {user.expertMode &&
        <div>
          <div className="w-two-thirds dib v-mid">
            <input type="text" placeholder="Filter tasks by id or username" className="pa2 w-100"
              value={textSearch || ''}
              onChange={e => setTextSearch(e.target.value)}
            />
          </div>
          <div className="w-third dib pl2">
            <Dropdown
              onAdd={() => {}}
              onRemove={() => {}}
              ...
              className="blue-dark bg-white mr1 v-mid pv2 ph3 ba b--grey-light"
            />
          </div>
        </div>
      }
    ...
    >
      {!tasksError && !tasksLoading &&
        <PaginatedList
          pageSize={6}
          items={sortBy === 'date' ? readyTasks.sort(compareLastUpdate) : readyTasks.sort(compareTaskId)}
...
function PaginatedList({items, ItemComponent, pageSize, projectId, selectTask, selected}: Object) {
  ...
  return <>
    ...
    <div className="fr">
      <PaginatorLine activePage={page || 1} setPageFn={setPage} lastPage={lastPage} className="flex items-center pt2" />
    </div>
  </>;
}
