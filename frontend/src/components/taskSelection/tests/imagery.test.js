...
it('test if Imagery returns the correct FormattedMessage to WMS', () => {
  ...
  const testInstance = element.root;
  expect(testInstance.findByType(FormattedMessage).props.id).toBe('project.imagery.wms');
});
...
it('test if Imagery returns the correct FormattedMessage to WMTS', () => {
  ...
  const testInstance = element.root;
  expect(testInstance.findByType(FormattedMessage).props.id).toBe('project.imagery.wmts');
});
...
it('test if Imagery returns the correct FormattedMessage to custom layer', () => {
  ...
  const testInstance = element.root;
  expect(testInstance.findByType(FormattedMessage).props.id).toBe('project.imagery.customLayer');
});

it('test if Imagery returns the correct imagery layer name', () => {
  const element = createComponentWithIntl(<Imagery value={'Mapbox Satellite'} />);
  const testInstance = element.root;
  expect(testInstance.findByType('span').children).toEqual(['Mapbox Satellite']);
});

it('test if Imagery returns the correct FormattedMessage to undefined imagery', () => {
  const element = createComponentWithIntl(<Imagery value={null} />);
  const testInstance = element.root;
  expect(testInstance.findByType(FormattedMessage).props.id).toEqual('project.imagery.noDefined');
});
