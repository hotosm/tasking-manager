...
describe('test TaskFilter with userCanValidate TRUE', () => {
  ...
  it('render 3 buttons and the first one is active', async () => {
    render(
    ...
    expect(screen.getByText('All').className).toContain('dbi bg-blue-grey white');
    expect(screen.getByText('Available for mapping').className).toContain('dbi bg-white blue-grey');
    expect(screen.getByText('Ready for validation').className).toContain('dbi bg-white blue-grey');
  ...
  it('Available for mapping is active and then click on Ready for validation', () => {
    ...
    expect(screen.getByText('All').className).toContain('dbi bg-white blue-grey');
    expect(screen.getByText('Available for mapping').className).toContain('dbi bg-blue-grey white');
    expect(screen.getByText('Ready for validation').className).toContain('dbi bg-white blue-grey');
    fireEvent.click(screen.getByText('Ready for validation'));
    expect(statusFilter).toBe('readyToValidate');
  });

  it('Ready for validation is active and then click on All', () => {
    render(
      <IntlProvider locale="en">
        <TaskFilter userCanValidate={true} statusFilter={statusFilter} setStatusFn={setStatus} />
      </IntlProvider>,
    ...
    expect(screen.getByText('All').className).toContain('dbi bg-white blue-grey');
    expect(screen.getByText('Available for mapping').className).toContain('dbi bg-white blue-grey');
    expect(screen.getByText('Ready for validation').className).toContain('dbi bg-blue-grey white');
