...
describe('FileRejections', () => {
  ...
  it('renders a li element to each file', () => {
    const files = [
      {
        ...
        errors: [
          ...
          { code: 'file-too-large', message: 'Size bigger than 100kb' },
        ],
      },
      {
    ...
    expect(screen.queryAllByText(/Format not supported/)[0].className).toBe('dib pr2 pl2');
    // expect(screen.queryByText(/Size bigger than 100kb/).className).toBe('dib pr2 pl2'); // added a custom message for bigger sized
  });
});
