...
import {
 ...
 const _OrderByPicker = props => {
    ...
    const fullOrderByNav =  (
        <nav ref={navRef} className={`di tc mt1 ba b--grey-light br1 fixed shadow-1 z-3 bg-white flex flex-column`}>
        <select onChange={onChangeSortBy} id="orderBy" className="ma3">
            ...
            <option value="mapper_level">{intl.formatMessage({...messages.mappingDifficulty})}</option>
            ...
            <option value="last_updated">{intl.formatMessage({...messages.sortBy_last_updated})}</option>
            <option value="due_date">{intl.formatMessage({...messages.sortBy_due_date})}</option>
        </select>
    
        <div className="pa3">
        <button onClick={() => onChangeSortByType("ASC") } title="sort ascending"><ChevronUpIcon/> </button>
        <button onClick={() => onChangeSortByType("DESC")} title="sort descending"><ChevronDownIcon/> </button>
        </div>
