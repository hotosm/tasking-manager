...
export const MappingTypeFilterPicker = props => {
    ...
    const handleMappingTypeInputChange = (event) => {
        ...
        const toggledType = props.mappingTypes && props.mappingTypes.find((n) => n === value) ? [] : [value]
        props.setMappingTypesQuery(
          otherTypes.concat(toggledType)
          , 'pushIn');
    }


    return (<div className="tc ma2 base-font">
        {
          props.titledIcons.map((EachIcon, key) => {
            const isActive = props.mappingTypes ? props.mappingTypes.find((n) => n === EachIcon.value) : false;
            const highlight = isActive ? activeStyle : inactiveStyle;
            return (
              <label title={EachIcon.title} key={"lblmtp"+key} className="relative">
                <input name="types" value={EachIcon.value}
