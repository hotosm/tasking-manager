...
return (
    ...
    <p className={`blue-grey ml2 f7`}>
        ...
        (state.isLoading) ?
          <span>&nbsp;</span> : 
          (!state.isError && <FormattedMessage
            {...messages.showingXProjectsOfTotal}
            values={{
              numProjects: state.projects && state.projects.length,
              numRange: state.pagination && state.pagination.page > 1  && (state.pagination.page * state.pagination.perPage) <= state.pagination.total && [": ",(state.pagination.page * state.pagination.perPage)," "].join(""),
              numTotalProjects: <FormattedNumber value={state.pagination && state.pagination.total}/>
            }}
          />)
        }
      </p>
        {state.isError ? (
          ...
          />
            <div className="pa2">
                <button className="pa1" onClick={() => props.retryFn()} >Retry</button>
            </div>
          </div>
        ) : null}
        <div className="cf mh5 mh2-ns db">
            <ReactPlaceholder customPlaceholder={nCardPlaceholders(5, cardWidthClass)}  ready={!state.isLoading}>
...
function ExploreProjectCards({pageOfCards}: Object, cardWidthClass: String) {
    ...
    return pageOfCards.map((card, n) => <ProjectCard cardWidthClass={cardWidthClass} { ...card } key={n} />);
  }
