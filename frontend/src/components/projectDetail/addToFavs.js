    ...
    return (
      <>
        <button
          ...
          className={`${!props.projectId ? 'dn' : ''} input-reset dim base-font bg-white blue-dark button-reset f6 bn pn`}
        >
        <FlagIcon className={`pt3 pr2 ${isLoading? 'o-50' : ''} ${isFav? 'red' : ''}`} />
        {state.isLoading? '...':''} {isFav? <FormattedMessage {...messages.removeFromFavorites} />: <FormattedMessage {...messages.addToFavorites} /> }
        </button>
