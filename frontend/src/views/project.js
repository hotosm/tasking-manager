...
import { useProjectsQueryAPI, useExploreProjectsQueryParams, stringify } from '../hooks/UseProjectsQueryAPI'
...
export const ProjectsPage = props => {
  ...
  const searchResultWidth = isMapShown ? "w-60-ns" : "w-100" ;
  ...
  return (
    <div className="pt180 pull-center">
      <ProjectNav 
        ...
        // campaignAPIState={campaignAPIState}
       >
        {props.children 
      ...
      <section className="flex">
        <ProjectSearchResults
          state={state}
          retryFn={forceUpdate}
          className={`${searchResultWidth}`}
        />
        {isMapShown && <ProjectsMap 
          state={state} 
          fullProjectsQuery={fullProjectsQuery}
          setQuery={setProjectQuery} 
          className={`dn dib-ns w-40`}
         />}
      </section>
...
export const MoreFilters = props => {
   ...
   return (
      ...
      <div className={leftpanelDivStyle}>
       ...
       <div 
        onClick={() => props.navigate(`/contribute${stringify(fullProjectsQuery) ? ["?",stringify(fullProjectsQuery)].join(""):""}`)}
