...
def upgrade():
    conn = op.get_bind()
    campaign_tags = conn.execute(sa.text("select distinct(campaigns) from tags where campaigns is not null"))
    ...
    for campaign_tag in campaign_tags:
        ...
        new_campaign_id = conn.execute("select id from campaigns where name ='" + campaign_tag + "'").scalar()
        projects = conn.execute("select id from projects " + " where campaign_tag='" + campaign_tag + "'")
...
def downgrade():
    ...
    op.create_index("ix_projects_campaign_tag", "projects", ["campaign_tag"], unique=False)
    op.create_table(
        "tags",
    ...
    for campaign_id, campaign_tag in campaigns:
        conn.execute(sa.text("insert into tags (campaigns) values ('" + campaign_tag + "')"))
        projects = conn.execute(
            sa.text("select project_id from campaign_projects where campaign_id=" + str(campaign_id))
        )
        for project in projects:
            project_id = project[0]
            conn.execute(
                sa.text("update projects set campaign_tag='" + campaign_tag + "' where id=" + str(project_id))
            )
        conn.execute(sa.text("delete from campaign_organisations where campaign_id=" + str(campaign_id)))
        conn.execute(sa.text("delete from campaign_projects where campaign_id=" + str(campaign_id)))
