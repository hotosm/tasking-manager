#!/bin/bash
ARNS=$(aws rds describe-db-instances --query "DBInstances[].DBInstanceArn" --output text --region ap-southeast-1)
for line in $ARNS; do
    TAGS=$(aws rds list-tags-for-resource --resource-name "$line" --query "TagList[]" --region ap-southeast-1)
