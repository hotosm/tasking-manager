...
class OsmReportService():
    def report_data_to_osm(self, project_dto: ProjectDTO):
        try:
            # Format project data
            project = self.format_project_data(project_dto)
            report_data = self.generate_project_report_data(project)
            osm_report_dto = OsmReportDTO(report_data)
            osm_report_dto.validate()
            osm_report_data = osm_report_dto.to_primitive(role="report")

            # current_app.logger.debug(osm_report_data)
            r =requests.post("http://0.0.0.0:5001/document/",
